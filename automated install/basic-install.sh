...
EOF
        ...
        if ! apk add --no-cache build-base bind-tools binutils coreutils curl git grep iproute2-ss jq libcap logrotate ncurses nmap-ncat procps-ng psmisc shadow tzdata unzip wget >/dev/null 2>&1; then
...
installCron() {
    ...
    install -Dm 644 -T -o root -g root ${PI_HOLE_LOCAL_REPO}/advanced/Templates/pihole.cron /etc/cron.d/pihole
